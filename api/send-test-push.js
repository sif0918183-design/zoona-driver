...
export default async function handler(req, res) {
    ...
    try {
        ...
        console.log('🔔 طلب إرسال إشعار تجريبي FCM:', { driverId });
        ...
        if (dbError || !driver || !driver.fcm_token) {
            return res.status(404).json({
                ...
                message: 'لم يتم العثور على توكن FCM لهذا السائق'
            });
        }
        ...
        console.log('✅ تم إرسال إشعار تجريبي FCM بنجاح للسائق:', driver.full_name);

        // 4. تسجيل العملية في Supabase
        await tarhalDB.from('push_notification_logs').insert({
            driver_id: driverId,
            success: true,
            notification_type: 'test_fcm',
            sent_at: new Date().toISOString()
        });

        return res.status(200).json({
            success: true,
            message: 'تم إرسال الإشعار التجريبي بنجاح عبر FCM'
        });
        ...
        return res.status(500).json({
            success: false,
            error: error.message,
            message: 'حدث خطأ في الخادم أثناء إرسال الإشعار التجريبي'
        });
    }
}
